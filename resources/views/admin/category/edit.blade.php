    ...
    <div class="row">
        <div class="col-md-3">
            <div class="row">
                <div class="col-md-12">
                    <div class="card-box">
                        <h4 class="m-t-0 m-b-30 header-title">{{trans('form.edit_category')}}</h4>
                        {!! Form::model($category, ['method'=>'PATCH', 'action'=>['AdminCategoryController@update', $category->id]]) !!}
                        {{csrf_field()}}
                        <div class="form-group">
                            <input name="name" type="text" class="form-control" value="{{$category->name}}" placeholder="{{trans('form.name')}}">
                        </div>
                        <button type="submit" class="btn btn-info btn-custom waves-effect w-xs waves-light m-b-5">{{trans('form.edit')}}</button>
                        {!! Form::close() !!}
                    </div><!-- end cardbox--></div>
